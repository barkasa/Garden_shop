...
//     e.preventDefault();
//     if (!isValidNumber) {
//       return; // Прерывание отправки формы при невалидном номере
//     }
//     // console.log(e.target.phone.value)
//     send_order(e.target.phone.value);
//     e.target.reset();
//     //
//     dispatch(resetCounterAction()); // Сброс счетчика после отправки заказа
//     dispatch(clearCartAction()); // Очистка корзины после отправки заказа
//     //
//   };
...
//   const discountedTotal = discountApplied ? total_sum * 0.95 : total_sum; // Apply discount if it's applied
...
//                   isCartEmpty ? s.disabled : ""
//                 }`}
//                 // >{`${total_sum.toFixed(2)}$`}</p>
//               >{`${discountedTotal.toFixed(2)}$`}</p>
//             </div>
//             {!isCartEmpty && (
//               <form onSubmit={submit} className={isCartEmpty ? s.disabled : ""}>
//                 <input
//                   // className={s.phone_input}
//                   className={`${s.phone_input} ${isValidNumber ? "" : s.error}`}
//                   type="text"
//                   placeholder="+49"
//                   name="phone"
//                   value={phoneNumber}
